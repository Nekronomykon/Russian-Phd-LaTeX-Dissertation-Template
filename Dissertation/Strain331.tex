...
\toprule\multicolumn{3}{c|}{Структура / симметрия} & \(E_{2}\),~\si{\hartree} & \(E_{ZPE}\),~\si{\hartree} & Геометрия \endfirsthead
\multicolumn{6}{p{\textwidth}}{\emph{продолжение таблицы}~\arabic{table}} \\
\toprule\multicolumn{3}{c|}{Структура / симметрия} & \(E_{2}\),~\si{\hartree} & \(E_{ZPE}\),~\si{\hartree} & Геометрия \endhead
\midrule\multicolumn{6}{l}{\emph{продолжение на следующей странице}} \endfoot
\bottomrule \multicolumn{6}{l}{} \endlastfoot
\midrule\cmpd{Propane} & \DrawMeEt{} & \(\SymGroup{C}{2v}\) & \num{-118.859638} & \num{0.104589} &
...
За исключением простейших структур \cmpd{Propane}--\cmpd{Acetone}, молекулы реагентов обладают конформационной подвижностью (карбонильная группа сама по себе вообще считается жёстко планарной; существующие исключения будут рассмотрены ниже). Инверсия атома азота триметиламина~\cmpd{Trimethylamine} слабо взаимодействует со внутренним вращением метильных групп, а (N,N-диметил)этиламина~\cmpd{Me2NEt} "--- преимущественно с внутренним вращением, параметризуемым подструктрурой \ce{H3C-N-CH2-CH3}...
...
\toprule\multicolumn{2}{c|}{Структура}  & \(E_{2}\),~\si{\hartree} & \(E_{ZPE}\),~\si{\hartree} & Геометрия \endfirsthead
  ...
  \midrule
  \cmpd{Cyclohexane} & \chemfig{?-[:+160]-[:-120]<[:+20]-[:-20,,,,line width=\boldbondwidth]>[:+60]?} / \SymGroup{D}{3d} &  \num{-235.344605} & \num{0.171358}& \ce{C-C}~\SI{1.526}{\angstrom}; $\angle\ce{CCC}~\ang{110.7}$; $\tau=\pm\ang{56.9}$ 
