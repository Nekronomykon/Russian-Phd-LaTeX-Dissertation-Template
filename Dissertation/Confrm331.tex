...
\newcommand{\ScaleCorrStr}{0.725}
\newcommand{\ScaleCorrAux}{0.825}
\begin{figure*}
  \caption{Конформационная энергия для некоторых вариантов симметричного гетероаналогичного замещения в бицикло[3.3.1]нонане,~ккал/моль (расчёт MP4(2)/aug-cc-pVDZ + ZPE)~\cite{Pisarev:2013:rus,Pisarev:2013}\label{fig:Conform331:MP2}}
  \centering
  \begin{tikzpicture}[xscale=3.75,yscale=1.75] %
    \DrawCorrFrame{-2}{6};
\draw(1.50,6.00) node [scale=\ScaleCorrAux,anchor=north,draw=black,fill=white]{\schemestart %
...
\draw(1.50,-1.00) node [scale=\ScaleCorrAux,anchor=north,draw=black,fill=white]{\schemestart %
...
\draw(3.85, 2.95) node [anchor=east]  {\cmpd{Bicyclo331:37O2:15N2}};
    %
\draw(4.00, 1.55) node [anchor=north] {\chemfig{[:-30,\ScaleCorrStr]S*6(-(-[:180]-[:120]O-[:+60]-[:0]?)--O---)}};
\draw(4.00, 1.55) node [anchor=south] {\num{1.55}};
\draw(3.85, 1.55) node [anchor=east]  {\cmpd{Bicyclo331:37O2:9S}};
    %
...
\draw(3.85,-1.53) node [anchor=east]  {\cmpd{Bicyclo331:37S2:9O}};
    %
  \end{tikzpicture}
\end{figure*}
