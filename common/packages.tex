...
\newcommand{\findmin}[2]{
  % Starting value for min : 0
  \pgfmathtruncatemacro{\mymin}{0}
  
  % Parsing each element of the first column
  \pgfplotsinvokeforeach {0,...,5}{
    \pgfplotstablegetelem{##1}{#1}\of{\mytable}
    \ifthenelse{ \pgfplotsretval < \mymin }
    {\pgfmathtruncatemacro{\mymin}{\pgfplotsretval}} % valid
    {}  %invalid
  }
  \let\ymin=\mymin%
}
\pgfplotstableset{%
  ...
  columns/Tau/.style={int detect,column type=r,column name={$\tau,~\si{\degree}$}},
  columns/Etot/.style={fixed,fixed zerofill,precision=6,column type=r,column name={$E_{tot},~\si{\hartree}$}},
  % columns/Erel/.style={}
  %create on use/Emin/.style={create col/copy=Etot},
  columns/Emin/.style={fixed,fixed zerofill,precision=6,
    column name={$E_0,~\si{\hartree}$},
    % create col/expr accum={\ifthenelse{\thisrow{Etot} < \pgfmathaccuma}{\thisrow{Etot}}{\pgfmathaccuma}}{0}
  },
  %create on use/Erel/.style={create col/copy=Emin},
  columns/Erel/.style={fixed,fixed zerofill,precision=6,showpos,
    column name={$\Delta\Func{E},~\si{\kilo\joule\per\mole}$},
